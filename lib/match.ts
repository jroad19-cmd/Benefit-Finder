...
export function findMatches(profile: Profile): MatchResult[] {
  ...
  const results = programs.map((program) => {
    ...
    addReason(!program.monthlyIncomeMax || profile.monthlyIncome <= program.monthlyIncomeMax, scoreRef, reasons, 'Monthly income appears within the target range.', 16);
    addReason(!program.annualIncomeMax || annualIncome <= program.annualIncomeMax, scoreRef, reasons, 'Annual income appears within the target range.', 16);
    ...
    if (program.monthlyIncomeMax && profile.monthlyIncome > program.monthlyIncomeMax) scoreRef.value -= 24;
    if (program.annualIncomeMax && annualIncome > program.annualIncomeMax) scoreRef.value -= 24;
